...
class TrainConfig:
    ...
    adam_iters: int = 3000
    adap_every: int = 600   # m=5
    lbfgs_iters: int = 2500
    lr: float = 1.0e-3
    ...
    print_every: int = 100
    save_dir: str = os.path.join(BASE_DIR, "models_harmonic_wave")
...
def exact_solution(x, t):
    ...
    phase = k * (x - PROBLEM.velocity * t)
    decay = torch.exp(-PROBLEM.diffusion * k**2 * t)
    return 0.5 + 0.5 * torch.sin(phase) * decay


def initial_condition(x):
    return exact_solution(x, torch.zeros_like(x))
...
class SingleStepPINN:
    def __init__(self, t0, x_ic, phi_ic, prev_model=None, step_id=0):
        ...
        self.X_left, self.X_right = self.sample_periodic_boundary()
    ...
    def sample_periodic_boundary(self):
        ...
        X_left = self._make_xt(x_left, t).detach().requires_grad_(True)
        X_right = self._make_xt(x_right, t).detach().requires_grad_(True)
    ...
    def periodic_loss(self):
        phi_left = self.model(self.X_left)
        phi_right = self.model(self.X_right)

        grad_left = torch.autograd.grad(
            phi_left,
            self.X_left,
            torch.ones_like(phi_left),
            create_graph=True,
        )[0][:, 0:1]
        grad_right = torch.autograd.grad(
            phi_right,
            self.X_right,
            torch.ones_like(phi_right),
            create_graph=True,
        )[0][:, 0:1]

        loss_value = torch.mean((phi_left - phi_right) ** 2)
        loss_grad = torch.mean((grad_left - grad_right) ** 2)
