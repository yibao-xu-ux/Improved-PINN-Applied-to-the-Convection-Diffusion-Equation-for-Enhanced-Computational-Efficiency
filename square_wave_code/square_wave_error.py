...
REPORT_DIR = os.path.join(BASE_DIR, "square_wave_reports")
...
def predict_at_time(x_values, t):
    ...
    with torch.no_grad():
        ...
        exact = exact_values(x, time)
    return pred.cpu().numpy(), exact.cpu().numpy()


def evaluate_on_grid(nx=600, nt=241):
...
def save_metrics(metrics):
    ...
    with open(path, "w", encoding="utf-8") as file:
        file.write("Square wave PINN error metrics\n")
        file.write("==============================\n")
