...
REPORT_DIR = os.path.join(BASE_DIR, "harmonic_wave_2d_reports")
...
def save_metrics(metrics, nx, ny, nt):
    ...
    with open(path, "w", encoding="utf-8") as file:
        file.write("2D harmonic wave PINN error metrics\n")
        file.write("===================================\n")
        file.write(f"Model directory: {MODEL_DIR}\n")
        file.write(f"Domain: x,y in [0, {PROBLEM.length:.12f}], t in [0, {TRAIN.t_end}]\n")
...
def plot_time_error(t_values, relative_l2, max_abs):
    ...
    ax_l2.set_title("2D harmonic wave error evolution")
