...
import harmonic_wave_2d_train as train_module
...
FIG_DIR = os.path.join(BASE_DIR, "harmonic_wave_2d_figures")
...
def plot_field_triptychs():
    ...
    for t in times:
        ...
        fig.suptitle(rf"2D harmonic wave at $t={t:.2f}$", y=1.04, fontsize=14)
        fig.savefig(os.path.join(FIG_DIR, f"field_triptych_t_{t:.2f}.png"))
        plt.close(fig)
...
def plot_surface_comparison():
    x = np.linspace(0.0, PROBLEM.length, 100)
    y = np.linspace(0.0, PROBLEM.length, 100)
    t = TRAIN.t_end
    X, Y, pred, exact = predict_on_grid(x, y, t)
    abs_error = np.abs(pred - exact)
